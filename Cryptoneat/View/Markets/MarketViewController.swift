...
class MarketViewController: UIViewController {
    ...
    //TODO: - MAKE VIEWSTATE AND LOADING VIEW CONTROLLER WORK
    private lazy var viewState: ViewState = .loading {
        didSet  {
            switch viewState {
                case .loading:
                    break
    ...
    func configureUI() {
        ...
        let barButtonImage = viewModel.isFavorite ? UIImage(systemName: "star.fill") : UIImage(systemName: "star")
        let item = UIBarButtonItem(image: barButtonImage, style: .plain, target: self, action: #selector(favoriteButtonDidTap(_:)))
    ...
    func configureChartViewController() {
        ...
        ///Set the bool variant to check whether the chart is for the last 24h or not
        ///
        chartViewController.is24hChart = segmentedControl.selectedSegmentIndex == 0 ? true : false
    ...
    func removeChartViewController() {
       var controller = UIViewController()
        ...
        {
            if controllers is MarketChartViewController {
    ...
    @objc func favoriteButtonDidTap(_ sender: UIBarButtonItem!) {
       let viewModel = MarketViewModel(market: market)
        if viewModel.isFavorite {
            viewModel.removeFromFavorites { (success) in
                if success {
            ...
            viewModel.addToFavorites { (success) in
                if success {
